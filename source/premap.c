...
extern Mix_Chunk * FX_Samples[NUM_SOUNDS + 11];

extern unsigned char everyothertime;
...
if (!VOLUMEONE) {
    ...
    {
#if 1
        /* try to unload all Mix_Chunks (free some memory for timidity) */
        unsigned int i;
        for (i=0; i<NUM_SOUNDS; i++)
        {
            if (FX_Samples[i])
            {
                Mix_FreeChunk(FX_Samples[i]);
                FX_Samples[i] = 0;
            }
        }
#endif
        dofrontscreens("Generating music data... wait, please.");
