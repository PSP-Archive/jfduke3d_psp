...
char unsigned playerswhenstarted;
char qe,cp;
...
unsigned char cheatbuf[10],cheatbuflen;
void cheats(void)
{
...
void Logo(void)
{
	...
	if (!VOLUMEALL) {
		...
		if(!KB_KeyWaiting() && nomorelogohack == 1)
		{
			getpackets();
			playanm("logo.anm",5);
			IFISSOFTMODE palto(0,0,0,63);
...
void getnames(void)
{
     ...
     if(numplayers > 1)
     {
		  ...
		  for(i=0;myname[i];i++) buf[l++] = toupper(myname[i]);
		  buf[l++] = 0;
...
void dobonus(char bonusonly)
{
    ...
    while( 1 )
    {
        ...
        if(ps[myconnectindex].gm&MODE_EOL)
        {
			...
			if( ( (MOUSE_GetButtons()&7) || KB_KeyWaiting() || KB_KeyPressed(sc_Enter)) && totalclock > (60*2) )	// JBF 20030809
            {
		MOUSE_ClearButton(7);
                if( KB_KeyPressed( sc_F12 ) )
                {
