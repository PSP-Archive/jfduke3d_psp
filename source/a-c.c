...
void hlineasm4(long cnt, long skiploadincs, long paloffs, unsigned long by, unsigned long bx, long p)
{
	...
	palptr = (unsigned char *)&ghlinepal[paloffs];
	if (!skiploadincs) { gbxinc = asm1; gbyinc = asm2; }
	for(;cnt>=0;cnt--)
	{
		*((unsigned char *)p) = palptr[gbuf[((bx>>(32-glogx))<<glogy)+(by>>(32-glogy))]];
		bx -= gbxinc;
		by -= gbyinc;
		p--;
	}
...
void slopevlin(long p, long i, long slopaloffs, long cnt, long bx, long by)
{
	...
	for(;cnt>0;cnt--)
	{
		...
		u = bx+globalx3*i;
		v = by+globaly3*i;
		(*(unsigned char *)p) = *(unsigned char *)(slopalptr[0]+gbuf[((u>>(32-glogx))<<glogy)+(v>>(32-glogy))]);
		slopalptr--;
		p += gpinc;
	}
}
...
void vlineasm1(long vinc, long paloffs, long cnt, unsigned long vplc, long bufplc, long p)
{
	...
	for(;cnt>=0;cnt--)
	{
		*((unsigned char *)p) = gpal[gbuf[vplc>>glogy]];
		p += bpl;
		vplc += vinc;
	}
}
...
void tvlineasm1(long vinc, long paloffs, long cnt, unsigned long vplc, long bufplc, long p)
{
	...
	if (transmode)
	{
		...
		{
			ch = gbuf[vplc>>glogy];
			if (ch != 255) *((unsigned char *)p) = gtrans[(*((unsigned char *)p))+(gpal[ch]<<8)];
			p += bpl;
			vplc += vinc;
		}
	}
	else
	{
		...
		{
			ch = gbuf[vplc>>glogy];
			if (ch != 255) *((unsigned char *)p) = gtrans[((*((unsigned char *)p))<<8)+gpal[ch]];
			p += bpl;
			vplc += vinc;
		}
	}
...
void mhline(long bufplc, unsigned long bx, long cntup16, long junk, unsigned long by, long p)
{
	...
	for(cntup16>>=16;cntup16>0;cntup16--)
	{
		...
		if (ch != 255) *((unsigned char *)p) = gpal[ch];
		bx += asm1;
		by += asm2;
		p++;
	}
...
void spritevline(long bx, long by, long cnt, long bufplc, long p)
{
	...
	for(;cnt>1;cnt--)
	{
		(*(unsigned char *)p) = gpal[gbuf[(bx>>16)*glogy+(by>>16)]];
		bx += gbxinc;
		by += gbyinc;
		p += bpl;
	}
...
void drawslab (long dx, long v, long dy, long vi, long vptr, long p)
{
	...
	while (dy > 0)
	{
		for(x=0;x<dx;x++) *(unsigned char *)(p+x) = gpal[(long)(*(unsigned char *)((v>>16)+vptr))];
		p += bpl; v += vi; dy--;
	}
}
...
void stretchhline (long p0, long u, long cnt, long uinc, long rptr, long p)
{
	...
	{
		p--;
		*(unsigned char *)p = *(unsigned char *)((u>>16)+rptr); u -= uinc;
	} while (p > p0);
}
