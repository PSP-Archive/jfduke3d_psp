...
PSP_HEAP_SIZE_KB(-1024); // Set Heapsize to MAX - 1MB
...
char nofog=0;
long deadzone=3200, saturzone=32767;
...
int initinput(void)
{
	...
	if (!SDL_InitSubSystem(SDL_INIT_JOYSTICK)) {
		...
		if (joydev) {
			...
			joyaxis = (long *)calloc(joynumaxes, sizeof(long));
			joyhat = (long *)calloc(joynumhats, sizeof(long));
		}
	}
...
void setjoydeadzone(int axis, unsigned short dead, unsigned short satur)
{
	...
	deadzone=((float)dead/10000.0f)*(float)32767;
	saturzone=((float)satur/10000.0f)*(float)32767;
	printf("setjoydeadzone(): axis %d, dead %d, satur %d", axis, deadzone, satur);
}
...
//	if (checkvideomode(&x,&y,c,fs) < 0) return -1;
	...
	sdl_surface = SDL_SetVideoMode(x, y, c,  SURFACE_FLAGS);
	if (!sdl_surface) {
	...
	while (SDL_PollEvent(&ev)) {
		switch (ev.type) {
			...
			case SDL_JOYAXISMOTION:
				...
				{
					if(( ev.jaxis.value < -deadzone ) || (ev.jaxis.value > deadzone ))
						joyaxis[ ev.jaxis.axis ] = ev.jaxis.value * 30000 / 32767;
					else
			...
			case SDL_JOYBUTTONUP:
				if(ev.jbutton.button==1)
					code = keytranslation[SDLK_LCTRL];

				//move around
				else if(ev.jbutton.button==7)
					code = keytranslation[SDLK_LEFT];
				else if(ev.jbutton.button==9)
					code = keytranslation[SDLK_RIGHT];
				else if(ev.jbutton.button==8)
					code = keytranslation[SDLK_UP];
				else if(ev.jbutton.button==6)
					code = keytranslation[SDLK_DOWN];

				//jump/crouch
				else if(ev.jbutton.button==0)
					code = keytranslation[SDLK_a];
				else if(ev.jbutton.button==3)
					code = keytranslation[SDLK_z];

				//strafe
				else if(ev.jbutton.button==4)
					code = keytranslation[SDLK_COMMA];
				else if(ev.jbutton.button==5)
					code = keytranslation[SDLK_PERIOD];

				else if(ev.jbutton.button==10)
					code = keytranslation[SDLK_ESCAPE];
				else if(ev.jbutton.button==11)
					code = keytranslation[SDLK_RETURN];
				else
					code = keytranslation[SDLK_SPACE];
