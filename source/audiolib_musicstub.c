...
Mix_Music *music = NULL; // <--- Mix music data for one song, you can only play
                          //      one song at a time anyway

int MUSIC_ErrorCode = MUSIC_Ok;

int MUSIC_Convert2File( unsigned char *ptr, long len );
...
/*---------------------------------------------------------------------
   ...
   Stops playback of current song.  (MrPeanut) - AND unloads it from
   memory.  You only stop a song when you want to unload it.  Pause
   it if you don't want to load a new song.
---------------------------------------------------------------------*/
...
int MUSIC_StopSong(void)
{
	...
	{
		Mix_HaltMusic();
		Mix_FreeMusic( music );  // free out global music data, this in turn stops it from playing (MrPeanut)
		music = NULL;
	}
	return( MUSIC_Ok );
}
...
/*---------------------------------------------------------------------
   ...
   Load, and play a MIDI song.  This is very processor intensive.
   Begins playback of MIDI song.
---------------------------------------------------------------------*/

int MUSIC_PlaySong( unsigned char *song, int loopflag, long len )
{
	if ( Mix_PlayingMusic() )
	{
		Mix_HaltMusic();
	...
	if( MUSIC_Convert2File( song, len ) != 1 )  // Mix_Loadmus requires the song to be in a file (in the psp atleast)
		return(0);

	music=Mix_LoadMUS( "tmp.mid" );
...
// MUSIC_Convert2File (MrPeanut)  SDL_RWops doesn't work with MIDI files, so we write
// a temporary MIDI file to the MS to load.

int MUSIC_Convert2File( unsigned char *ptr, long len )
{
	FILE *fin;

	fin = fopen( "tmp.mid", "wb" );
	if( !fin )
		exit( 0 );

	fwrite( ptr, len, 1, fin );
	fclose( fin );
	return( 1 );
}
